...
class NearestNeighborClassifier:
    ...
    def make_data(cls, x: list[list[float]], y: list[float]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return (torch.as_tensor(x, dtype=torch.float), torch.as_tensor(y, dtype=torch.float))
    ...
    def get_nearest_neighbor(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        idx = (self.data_normalized - x_norm).abs().sum(dim=1).argmin()
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        x_norm = self.input_normalization(x)
        k_near_idx = (self.data_normalized - x_norm).abs().sum(dim=1).topk(k, largest=False).indices
        nearest_x = self.data.index_select(0,k_near_idx)
        nearest_label = self.label.index_select(0,k_near_idx)
        return (nearest_x, nearest_label)
