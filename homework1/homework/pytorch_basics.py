...
class PyTorchBasics:
    ...
    def make_it_pytorch_4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        x_mean = torch.mean(x)
        return torch.where(y>x_mean, 1.0, 0.0).sum()
    ...
    def make_it_pytorch_9(x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.cumsum(torch.cumsum(x, 1), 0)
    ...
    def make_it_pytorch_11(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.nonzero(torch.where(x < c, 1, 0)).mT
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        result = torch.where(result<1e-3,1,0)
        return torch.count_nonzero(torch.count_nonzero(result, dim=0))
