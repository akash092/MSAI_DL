...
class WeatherForecast:
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        meandiff = (self.data-self.data.mean(dim=1, keepdim=True)).abs()
        row_max_indices = meandiff.argmax(dim=1)
        return self.data[torch.arange(self.data.shape[0]), row_max_indices]
